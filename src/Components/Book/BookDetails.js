...
export default class BookDetails extends Component {
    ...
    this.books
      ...
      .then(books => {
        let book = books.find(book => {
          return (
            book.title === this.state.item.title &&
            book.author === this.state.item.author
          );
        });
  ...
  onBookmark = () => {
    this.books
      .toggleBookmark(this.state.item)
      .then(() => {
        this.updateBooks();
      })
      .catch(error => {
    ...
    return (
      <View style={Styles.alternativesContainer}>
        <View style={Styles.container}>
          <Text style={Styles.heading}>Vaihtoehtoiset kirjat</Text>
        </View>
        {this.state.item.alternatives.map((entry, index) => {
          return (
            <View
              key={`alternative-item-${index}`}
              style={Styles.alternativesAvailability}
            >
              <Text style={Styles.wrappingText} key={`title-${index}`}>
                {entry.title}
              </Text>
              <TouchableOpacity
                  ...
                  Linking.canOpenURL(uri)
                    .then(supported => {
                      Linking.openURL(uri).catch(error => {
                        alert(
                          "Selainta ei voitu avata saatavuustietojen tarkistamiseksi."
                        );
                      });
                    })
                    .catch(error => {});
    ...
    return (
      <View style={Styles.mainContainer}>
        <ScrollView>
          ...
          {this.state.item.additionalInformation
            ? this.renderInformation(this.state.item.additionalInformation)
            : null}
          {this.state.item.alternatives.length > 0
